...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    //Finding first free block with size >= blocksize
    while (curr != NULL) {
    ...
    //Remove the chosen node from FREE_LIST
    block_t *blk = chosen->blk;
    int original_end = blk->end;

    if (chosen_prev == NULL) {
        //remove head
        freelist->head = chosen->next;
    }
    else{
    ...
    //Resize allocated block to requested size
    blk->end = blk->start + blocksize - 1;

    //Add blk to ALLoc_List sorted by address
    list_add_ascending_by_address(alloclist, blk);

    //Handle leftover fragment
    int remaining_size = original_end - blk->end;
    if (remaining_size > 0) {
        ...
        //Fragments inseted into FREE_LIST based on policy
        if(policy == 1) {
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    //find block in ALLOC_LIST with blk->pid == pid
    while (curr != NULL) {
        if (curr->blk->pid == pid) {
            chosen = curr;
            chosen_prev = prev;
    ...
    //add blk back to FREE_List according to policy
    if (policy == 1) {
